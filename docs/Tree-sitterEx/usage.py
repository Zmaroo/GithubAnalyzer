...
# Example of correct tree-sitter query patterns for Python
PYTHON_QUERY = """
;; Match complete functions
(function_definition
...
(ERROR
  [(function_definition
   ...
   ;; Match broken function definitions
   (expression_statement
     (call
       function: (identifier) @name)) @partial_def]) @error
"""

# Example of proper tree-sitter traversal
    ...
    while True:
        if not visited_children:
            ...
            if node.type == "function_definition":
                ...
                if name_node:
                    process_function(name_node)
            
            # Handle error nodes
            elif node.type == "ERROR":
                error_cursor = node.walk()
                process_error_node(error_cursor)
